...
export async function searchArxiv(term: string, signal?: AbortSignal): Promise<AcademicSource[]> {
  ...
  try {
    const response = await fetch(url, { headers: { Accept: 'application/atom+xml' }, signal });
...
export async function searchPubMed(term: string, context: string, signal?: AbortSignal): Promise<AcademicSource[]> {
  ...
  try {
    const ids = ((await (await fetch(search, { signal })).json()) as { esearchresult?: { idlist?: string[] } }).esearchresult?.idlist?.filter((id) => /^\d+$/.test(id)).slice(0, 6) || [];
    ...
    const response = await fetch(fetchUrl, { signal }); if (!response.ok) return [];
...
export async function searchCrossref(term: string, context: string, signal?: AbortSignal): Promise<AcademicSource[]> {
  ...
  try {
    const payload = (await (await fetch(url, { headers: { Accept: 'application/json' }, signal })).json()) as { message?: { items?: Array<Record<string, unknown>> } };
...
export async function retrieveSources(term: string, context: string, preference: DomainPreference, signal?: AbortSignal): Promise<AcademicSource[]> {
  const primary = preference === 'medicine' ? await searchPubMed(term, context, signal) : preference === 'ai' ? await searchArxiv(term, signal) : [];
  if (primary.length) return primary.map((source, index) => ({ ...source, id: index + 1 }));
  return (await searchCrossref(term, context, signal)).map((source, index) => ({ ...source, id: index + 1 }));
}
